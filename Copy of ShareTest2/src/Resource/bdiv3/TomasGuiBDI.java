 ...
 */
public class TomasGuiBDI extends JFrame
{
	//-------- attributes --------
	 ...
	 */
	public TomasGuiBDI(IExternalAccess agent)
	{
		...
		send.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				TomasGuiBDI.this.agent.scheduleStep(new IComponentStep<Void>()
				{
						...
						fut.addResultListener(new IIntermediateResultListener<IlogSevice>()
						{
							public void resultAvailable(Collection<IlogSevice> result)
							{
								for(Iterator<IlogSevice> it=result.iterator(); it.hasNext(); )
								{
									...
									try
									{
										cs.message(TomasGuiBDI.this.agent.getComponentIdentifier().getName(), text, text);
									}
									catch(Exception e)
									{
		...
		addWindowListener(new WindowAdapter()
		{
			...
			{
				TomasGuiBDI.this.agent.killComponent();
				TomasGuiBDI.this.agent	= null;
			}
		});
